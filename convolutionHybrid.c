               ...
               float* kernel, int kernelSizeX, int kernelSizeY)
{
    ...
    //printf("Thread: %i dataSizeX: %i kCenterY: %i kCenterX: %i\n", omp_get_thread_num(), dataSizeX, kCenterY, kCenterX);
    ...
    //printf("Convolution with thread %d\n", omp_get_thread_num());
        ...
        for(j = 0; j < dataSizeX; ++j)              // number of columns
        {
            ...
            for(m = 0; m < kernelSizeY; ++m)        // kernel rows
            {
                ...
                if(m <= rowMax && m > rowMin)
                {
                    ...
                    {
                        //printf("kPtr: %i thread: %i m: %i n: %i\n", &*kPtr, omp_get_thread_num(), m, n);
                        //printf("k: %f thread: %i\n", *kPtr, omp_get_thread_num());
...
//    int headstored=0, imagestored=0, stored;
    ...
    printf("Testing: rank %d of %d, name %s\n", rank, nprocs, hostname);
